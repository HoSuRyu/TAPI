...
{
    public class projectNameFilter : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            filterContext.Controller.ViewBag.조사명 = "2022년 장애인 경제활동 조사";
        }
    }

    [projectNameFilter]
    public class q220677Controller : Controller 
    {
        private int UID
        {
            get
        ...
        public List<SP_q220677_GETLISTResult> GetList(int? pid = null)
        {
            List<SP_q220677_GETLISTResult> ResultList = new List<SP_q220677_GETLISTResult>();
            ...
            {
                ResultList = TAPI.SP_q220677_GETLIST(UID, pid).ToList();
            }
        ...
        public ActionResult ProgressPartial()
        {
            ...
            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                ResultList = TAPI.SP_q220677_GETLIST(UID, null).ToList();
        ...
        public ActionResult Select(int pid)
        {
           ...
           q220677_List list = null;
            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                list = TAPI.q220677_List.First(f => f.PID == pid);
            }
        ...
        public string SaveInfo(int pid, string roadAddrPart1, string siNm, string sggNm, string emdNm, string sebuAddr, string IsNewAddress, string ctel, string relation, string IsNewTel)
        {
            ...
            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                q220677_List list = TAPI.q220677_List.First(f => f.PID == pid);
         ...
         public ActionResult Survey(int pid)
        {
            ...
            try
            {
                ...
                q220677_List list = new q220677_List();

                bool eqnum = false;

                using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                {
                    list = (from surveyInfo in TAPI.q220677_List
                    ...
                    if (list.최종접촉결과 != "조사협조")
                    {
                        List<q220677_Contact> ContactList = new List<q220677_Contact>();
                        ContactList = (from contact in TAPI.q220677_Contact
                        ...
                        q220677_Contact Contact = new q220677_Contact();
                        ...
                        TAPI.q220677_Contact.InsertOnSubmit(Contact);
                    ...
                    string query = string.Format(@"IF EXISTS (SELECT * FROM SURVEY.sys.objects WHERE object_id = OBJECT_ID(N'[SURVEY].[dbo].[T_SamplingList_q220677]') AND type in (N'U'))
BEGIN
    MERGE SURVEY.dbo.T_SamplingList_q220677 as SLIST
    USING
    (
        SELECT	'{0}' as PID
...
END", pid, QMListData);
                ...
                if (eqnum == false)
                {
                    ViewBag.C = TripleDESCryptoService.Encrypt(string.Format("qn=q220677&t=tapi&pid={0}&uid={1}&nv=true", pid, UID));
                }
                else
                {
                    string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12739").Replace("/q220677", ""), pid);
                    ...
                    using (SURVEYTOOLDataContext SURVEYTOOL = new SURVEYTOOLDataContext("Data Source=10.0.11.175,15000;Initial Catalog=SURVEYTOOL;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                    {
                        ...
                        T_Script t_Script = SURVEYTOOL.T_Scripts.Where(w => w.questionnaireID == 12739).First();
                        ...
                        if (t_Script != null)
                        {
                            ...
                            answerList = (from element in variableList.Elements()
                                          select new AnswerList
                                          {
                                              ...
                                              Url = TripleDESCryptoService.Encrypt(string.Format("qn=q220677&pid={0}&uid={1}&t=tapi&eqnum={2}&nv=true", pid, UID, element.Name.ToString())),
        ...
        public string ContactOK(int pid, int ranking, string contact, string etc, string contactMonth, string contactDay, string contactTime, string type)
        {
            if (UID <= 0)
            {
                return "에러. 재접속 해 주세요.";
            ...
            q220677_Contact Contact = new q220677_Contact();
            q220677_Replace Replace = new q220677_Replace();
            q220677_List List = new q220677_List();
            ...
            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                ...
                if (type == "대체요청")
                {
                    ...
                    TAPI.q220677_Replace.InsertOnSubmit(Replace);

                    // 일단 주석 처리
                    //    //////////_21수정 필요 부분 : 발달 함께 업데이트
                    //    q220678_List Q220678_List = new q220678_List();
                    //    Q220678_List = (from surveyInfo in TAPI.q220678_List
                    //                          where surveyInfo.PID == pid
                    //                          select surveyInfo).SingleOrDefault();

                    //    if (Q220678_List != null)
                    //    {
                    //        Q220678_List.진행상태 = "경활 탈락";
                    //        Q220678_List.최종접촉결과 = "경활 탈락(표본대체)";
                    //        Q220678_List.경활조사결과 = "대체요청";
             ...
             string address1, string address2, string emdNm, string address, string dongho, string edit_tel)
        {
            ...
            q220677_Contact Contact = new q220677_Contact();            
            q220677_List List = new q220677_List();
            q220677_Transfer Transfer = new q220677_Transfer();
            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                Contact.PID = pid;
                Contact.면접원아이디 = UID.ToString();
                ...
                TAPI.q220677_Transfer.InsertOnSubmit(Transfer);
