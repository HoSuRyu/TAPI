...
{
    public class q220674_4Controller : Controller
    {
        private int UID
        {
            get
        ...
        public List<SP_q220674_GETLISTResult> GetList()
        {
            ...
            {
                list = TAPI.SP_q220674_GETLIST(UID, 4, null).ToList();
            }
        ...
        public ActionResult SetInfo(string pid)
        {
            ...
            string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12752").Replace("/q220674_4", ""), pid);
        ...
        public ActionResult Survey(string pid)
        {
            ...
            try
            {
                ...
                using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                {
                    q220674_4_List list = TAPI.q220674_4_List.First(f => f.PID == pid);

                    if (string.IsNullOrEmpty(list.진행상태) == false && list.진행상태 == "재조사요청")
                    {
                        eqnum = true;
                    }
                    TAPI.SP_q220674_SETINFO(pid, 4, "진행중");
                    ...
                    string query = string.Format(@"IF EXISTS (SELECT * FROM SURVEY.sys.objects WHERE object_id = OBJECT_ID(N'[SURVEY].[dbo].[T_SamplingList_q220674_4]') AND type in (N'U'))
BEGIN
    MERGE SURVEY.dbo.T_SamplingList_q220674_4 as SLIST
    USING
    (
        SELECT	'{0}' as PID
...
END", pid, QMListData);
                ...
                if (eqnum == false)
                {
                    ...
                    ViewBag.C = TripleDESCryptoService.Encrypt($"qn=q220674_4&pid={pid}&uid={UID}");
                }
                else
                {
                    string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12752").Replace("/q220674_4", ""), pid);
                    ...
                    using (SURVEYTOOLDataContext SURVEYTOOL = new SURVEYTOOLDataContext("Data Source=10.0.11.175,15000;Initial Catalog=SURVEYTOOL;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                    {
                        ...
                        T_Script t_Script = SURVEYTOOL.T_Scripts.Where(w => w.questionnaireID == 12752).First();
                        ...
                        if (t_Script != null)
                        {
                            ...
                            answerList = (from element in variableList.Elements()
                                          select new AnswerList
                                          {
                                              ...
                                              Url = TripleDESCryptoService.Encrypt(string.Format("qn=q220674_4&pid={0}&uid={1}&eqnum={2}&nv=true", pid, UID, element.Name.ToString())),
