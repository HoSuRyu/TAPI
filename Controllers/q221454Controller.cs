...
{
    public class q221454Controller : Controller
    {
        private int UID
        {
            get
        ...
        public List<SP_q221454_GETLISTResult> GetList()
        {
            List<SP_q221454_GETLISTResult> list = new List<SP_q221454_GETLISTResult>();
            ...
            {
                list = TAPI.SP_q221454_GETLIST(UID, null).ToList();
            }
        ...
        public List<q221454_PersonList> GetPersonList(int keyid)
        {
            List<q221454_PersonList> list = new List<q221454_PersonList>();

            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                list = TAPI.q221454_PersonList.Where(f => f.DID == keyid && f.deleteYN == 0).ToList();

                q221454_DongList dongList = TAPI.q221454_DongList.First(f => f.DID == keyid);
                ViewBag.DID = keyid;
                ViewBag.전체주소정보 = dongList.전체주소정보;
                ViewBag.원대체 = dongList.원대체;
            }
        ...
        public string CreatePerson(int keyid)
        {
            ...
            try
            {
                using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                {
                    TAPI.SP_q221454_INSERTPERSON(UID, keyid);
        ...
        public ActionResult Survey(int keyid, int pid)
        {
            ...
            try
            {
                ...
                {
                    T_SamplingList_0 t_SamplingList = SURVEY.ExecuteQuery<T_SamplingList_0>(string.Format("SELECT * FROM T_SamplingList_q221454 WHERE PID = '{0}'", pid)).FirstOrDefault();
                ...
                using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                {
                    ...
                    string query = string.Format(@"IF EXISTS (SELECT * FROM SURVEY.sys.objects WHERE object_id = OBJECT_ID(N'[SURVEY].[dbo].[T_SamplingList_q221454]') AND type in (N'U'))
BEGIN
    MERGE SURVEY.dbo.T_SamplingList_q221454 as SLIST
    USING
    (
        SELECT	'{0}' as PID
...
END", pid, QMListData);
                    ...
                    q221454_PersonList PersonList = TAPI.q221454_PersonList.First(f => f.PID == pid);
                    if ((string.IsNullOrEmpty(PersonList.설문진행상태) == false && PersonList.설문진행상태 != "진행중"  ) ||  answerStateCode == 4 || answerStateCode==1 )
                    {
                        eqnum = true;
                    }

                    TAPI.SP_q221454_SETLIST(pid, "진행중", "","","","",null,null,null);
                }

                
                if (eqnum == false)
                {
                    ViewBag.C = TripleDESCryptoService.Encrypt($"qn=q221454&pid={pid}&did={keyid}&t=TAPI&atype=1");
                }
                else
                {
                    ViewBag.PID = pid.ToString();                    

                    string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\" + qID.ToString()).Replace("/q221454", ""), pid);
