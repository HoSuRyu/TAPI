...
{
    public class KLIPS2022Controller : Controller
    {
        private int UID
        {
            get
        ...
        public List<SP_KLIPS2022_GETLISTResult> GetList(long? hid = null)
        {
            List<SP_KLIPS2022_GETLISTResult> list = new List<SP_KLIPS2022_GETLISTResult>();
            ...
            {
                list = TAPI.SP_KLIPS2022_GETLIST(UID, hid).ToList();
                if (list.Count  > 0)
                {
        ...
        public ActionResult ProgressPartial()
        {
            ...
            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                list = TAPI.SP_KLIPS2022_GETLIST(UID, null).ToList();
        ...
        public ActionResult Contact(long hid)
        {
            if (UID <= 0)
            {
                return RedirectToAction("Login", "Home");
            ...
            List<KLIPS2022_Contact> ContactList = new List<KLIPS2022_Contact>();

            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                KLIPS2022_HouseList houseList = TAPI.KLIPS2022_HouseList.FirstOrDefault(f => f.HID == hid);


                ContactList = (from contact in TAPI.KLIPS2022_Contact
        ...
        public string ContactOK(long hid, int ranking, string contact, string contactMonth, string contactDay, string contactTime, string etc7, string etc8)
        {
            ...
            KLIPS2022_Contact Contact = new KLIPS2022_Contact();
            KLIPS2022_HouseList HouseList = new KLIPS2022_HouseList();
            

            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                Contact.HID = hid;
                Contact.면접원id = UID.ToString();
                ...
                TAPI.KLIPS2022_Contact.InsertOnSubmit(Contact);

                HouseList = (from surveyInfo in TAPI.KLIPS2022_HouseList
        ...
        public ActionResult Transfer(long hid)
        {
        ...
        public ActionResult TransferOK(long hid,  string address1, string address2, string emdNm, string address, string dongho, string edit_tel1, string edit_tel2, string reason, string reasonetc, string etc)
        {
            if (UID <= 0)
            {
                return RedirectToAction("Login", "Home");
            ...
            KLIPS2022_Transfer Transfer = new KLIPS2022_Transfer();
            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                ...
                HouseList.진행상태 = "이관요청";
                //주소 변경 여부 확인 필요
                //HouseList.시도 = address1;
                //HouseList.시군구 = address2;
                ////HouseList.읍면동 = emdNm;
                //HouseList.상세주소 = address + " " + dongho;
                ...
                TAPI.KLIPS2022_Transfer.InsertOnSubmit(Transfer);
                TAPI.SubmitChanges();
            }
            return null;
        }
        ...
        public ActionResult Agree(long hid)
        {
            ...
            KLIPS2022_HouseList houseList = null;

            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                houseList = TAPI.KLIPS2022_HouseList.First(f => f.HID == hid);
        ...
        public ActionResult AgreePerson(long hid, string pid, string surveyType )
        {
            ...
            KLIPS2022_HouseList houseList = null;
            KLIPS2022_PersonList personList = null;
            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                houseList = TAPI.KLIPS2022_HouseList.First(f => f.HID == hid);
                ViewBag.동의여부 = houseList.동의여부;
                ...
                personList = TAPI.KLIPS2022_PersonList.First(f => f.PID == pid);
        ...
        public ActionResult AnswerInfo(long hid)
        {
            if (UID <= 0)
            {
                return RedirectToAction("Login", "Home");
            ...
            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            {
                ...
                KLIPS2022_HouseList houseList = TAPI.KLIPS2022_HouseList.First(f => f.HID == hid);
                houseList.동의여부 = "동의";

                int personC = TAPI.KLIPS2022_PersonList.Count(c => c.HID == hid.ToString() && c.deleteYN==0);               
                int pAnswerC = TAPI.KLIPS2022_PersonList.Count(c => c.HID == hid.ToString() && c.deleteYN == 0 && (c.최종진행상태 == "완료" || c.최종진행상태 =="설문종료" || c.최종진행상태 =="탈락"));
        ...
        /// <returns></returns>
        public List<SP_KLIPS2022P_GETLISTResult> GetPersoninfo(long hid)
        {
            List<SP_KLIPS2022P_GETLISTResult> list = new List<SP_KLIPS2022P_GETLISTResult>();

            using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
            { 
                list = TAPI.SP_KLIPS2022P_GETLIST(hid).ToList();
                ...
                {
                    string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12561"), hid);
                    if(houseList.유형 == "분가")
                    {
                        dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12566"), hid);
                    }
                    ...
                    if (System.IO.File.Exists(dataFile) == true)
                    {
                        ...
                        for (int i = 1; i <= 20; i++)
                        {
                            ...
                            if( ptype == "신규합가")
                            {
                                ...
                                TAPI.SP_KLIPS2022_SETJOINHOUSE(ppid, oldPid);
                            }
                            else if (houseList.유형 == "분가")
                            {
                                string oldpid = "";
                                KLIPS2022_SplitHouseList ShouseList = (from house in TAPI.KLIPS2022_SplitHouseList
                            ...
                            int personYN = TAPI.KLIPS2022_PersonList.Count(c => c.PID == ppid);
                            KLIPS2022_PersonList personInfo = null;
                            if (personYN == 1)
                            {
                                personInfo = (from surveyInfo in TAPI.KLIPS2022_PersonList
                            ...
                            {
                                personInfo = new KLIPS2022_PersonList();
                                personInfo.HID = hid.ToString();
                                personInfo.PID = ppid;
                                personInfo.PersonIDX = i;
                                personInfo.이름 = pname;
                                ...
                                TAPI.KLIPS2022_PersonList.InsertOnSubmit(personInfo);
                            }
                        ...
                        List<SP_KLIPS2022P_GETLISTResult> personList = new List<SP_KLIPS2022P_GETLISTResult>();
                        personList = TAPI.SP_KLIPS2022P_GETLIST(hid).ToList();
                        ...
                        for(int i=1; i<=20; i++)
                        {
                            ...
                            if ( tperson == "" && ppid != "")
                            {
                                int isPerson = TAPI.KLIPS2022_PersonList.Count(c => c.PID == ppid);
                                if(isPerson != 0)
                                { 
                                    KLIPS2022_PersonList personInfo = (from surveyInfo in TAPI.KLIPS2022_PersonList
                                                                       where surveyInfo.PID == ppid
                                                                       select surveyInfo).SingleOrDefault();
            ...
            try
            {
                ...
                using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                {
                    ...
                    if (houseList.시작일자 == null || houseList.시작일자 == "")
                    {
                        ...
                        KLIPS2022_Contact Contact = new KLIPS2022_Contact();
                        int ranking = TAPI.KLIPS2022_Contact.Count(c => c.HID == hid ) + 1;
                        Contact.HID = hid;
                        Contact.면접원id = UID.ToString();
                        Contact.방문횟수 = ranking;
                ...
                if (eqnum == false)
                {
                    ViewBag.C = TripleDESCryptoService.Encrypt(string.Format("qn=q220355&t=tapi&pid={0}&uid={1}&nv=true", hid, UID));
                }
                else
                {
                    using (SURVEYTOOLDataContext SURVEYTOOL = new SURVEYTOOLDataContext("Data Source=10.0.11.175,15000;Initial Catalog=SURVEYTOOL;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                    {
                        ...
                        string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12561").Replace("/q220355", ""), hid);
                        ...
                        T_Script t_Script = SURVEYTOOL.T_Scripts.Where(w => w.questionnaireID == 12561).First();
                        ...
                        if (t_Script != null)
                        {
                            ...
                            answerList = (from element in variableList.Elements()
                                          select new AnswerList
                                          {
                                              ...
                                              Url = TripleDESCryptoService.Encrypt(string.Format("qn=q220355&pid={0}&uid={1}&t=tapi&eqnum={2}&nv=true", hid, UID, element.Name.ToString())),
        ...
        public ActionResult SplitSurvey(long hid)
        {
            ...
            try
            {
                ...
                if (eqnum == false)
                {
                    ViewBag.C = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_7&t=tapi&pid={0}&uid={1}&nv=true", hid, UID));
                }
                else
                {
                    string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12566").Replace("/q220355_7", ""), hid);
                    ...
                    using (SURVEYTOOLDataContext SURVEYTOOL = new SURVEYTOOLDataContext("Data Source=10.0.11.175,15000;Initial Catalog=SURVEYTOOL;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                    {
                        ...
                        T_Script t_Script = SURVEYTOOL.T_Scripts.Where(w => w.questionnaireID == 12566).First();
                        ...
                        if (t_Script != null)
                        {
                            ...
                            answerList = (from element in variableList.Elements()
                                          select new AnswerList
                                          {
                                              ...
                                              Title = element.Value,
                                              Answer = GetAnswerData(AnswerData, element.Name.ToString()),
                                              Url = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_7&pid={0}&uid={1}&t=tapi&eqnum={2}&nv=true", hid, UID, element.Name.ToString())),
        ...
        public ActionResult PersonSurvey(long pid)
        {
            ...
            try
            {
                ...
                KLIPS2022_PersonList personList = new KLIPS2022_PersonList();
                bool eqnum = false;
                int splitH = 0;
                ...
                using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                {
                    ...
                    personList = (from surveyInfo in TAPI.KLIPS2022_PersonList
                    ...
                    KLIPS2022_HouseList houseList = (from house in TAPI.KLIPS2022_HouseList
                    ...
                    TAPI.SP_KLIPS2022_SETINFO(pid, "진행중", 3);
                ...
                if (eqnum == false)
                {
                    ...
                    ViewBag.C = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_1&t=tapi&pid={0}&uid={1}&phid={3}&ppid2={4}&splith={2}&nv=true", pid, UID, splitH, personList.HID, pid2));
                }
                else
                {
                    string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12562").Replace("/q220355_1", ""), pid);
                    ...
                    else
                    {
                        using (SURVEYTOOLDataContext SURVEYTOOL = new SURVEYTOOLDataContext("Data Source=10.0.11.175,15000;Initial Catalog=SURVEYTOOL;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                        {
                            ...
                            T_Script t_Script = SURVEYTOOL.T_Scripts.Where(w => w.questionnaireID == 12562).First();
                            ...
                            if (t_Script != null)
                            {
                                ...
                                answerList = (from element in variableList.Elements()
                                              select new AnswerList
                                              {
                                                  ...
                                                  Title = element.Value,
                                                  Answer = GetAnswerData(AnswerData, element.Name.ToString()),
                                                  Url = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_1&pid={0}&uid={1}&t=tapi&eqnum={2}&phid={3}&ppid2={4}&nv=true", pid, UID, element.Name.ToString(), personList.HID, pid2)),
                                              }).Where(w => string.IsNullOrEmpty(w.Answer) == false).ToList();
                            }
        ...
        public ActionResult NewPersonSurvey(long pid)
        {
            ...
            try
            {
                ...
                using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                {
                    ...
                    string query = string.Format(@"IF EXISTS (SELECT * FROM SURVEY.sys.objects WHERE object_id = OBJECT_ID(N'[SURVEY].[dbo].[T_SamplingList_q220355_4]') AND type in (N'U'))
                    BEGIN
                        MERGE SURVEY.dbo.T_SamplingList_q220355_4 as SLIST
                        USING
                        (
                            SELECT	'{0}' as PID
                ...
                if (eqnum == false)
                {
                    ViewBag.C = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_4&t=tapi&pid={0}&uid={1}&pid2={2}&splith={3}&nv=true", pid, UID, pid2, splitH));
                }
                else
                {
                    string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12563").Replace("/q220355_4", ""), pid);
                    FileInfo fi = new FileInfo(dataFile);
                    if (fi.Exists == false)
                    {
                        ViewBag.C = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_1&t=tapi&pid={0}&uid={1}&phid={3}&ppid2={4}&splith={2}&nv=true", pid, UID, splitH, personList.HID, pid2));
                    ...
                    else
                    {
                        using (SURVEYTOOLDataContext SURVEYTOOL = new SURVEYTOOLDataContext("Data Source=10.0.11.175,15000;Initial Catalog=SURVEYTOOL;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                        {
                            ...
                            T_Script t_Script = SURVEYTOOL.T_Scripts.Where(w => w.questionnaireID == 12563).First();
                            ...
                            if (t_Script != null)
                            {
                                ...
                                answerList = (from element in variableList.Elements()
                                              select new AnswerList
                                              {
                                                  ...
                                                  Title = element.Value,
                                                  Answer = GetAnswerData(AnswerData, element.Name.ToString()),
                                                  Url = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_4&pid={0}&uid={1}&t=tapi&eqnum={2}&pid2={3}&nv=true", pid, UID, element.Name.ToString(), pid2)),
                                              }).Where(w => string.IsNullOrEmpty(w.Answer) == false).ToList();
                            }
                        }
                    }
        ...
        public ActionResult AttitudeSurvey(long pid)
        {
            ...
            try
            {
                ...
                if (eqnum == false)
                {
                    ViewBag.C = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_3&t=tapi&pid={0}&uid={1}&hid={2}&nv=true", pid, UID, hid));
                }
                else
                {
                    string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12564").Replace("/q220355_3", ""), pid);
                    ...
                    using (SURVEYTOOLDataContext SURVEYTOOL = new SURVEYTOOLDataContext("Data Source=10.0.11.175,15000;Initial Catalog=SURVEYTOOL;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                    {
                        ...
                        T_Script t_Script = SURVEYTOOL.T_Scripts.Where(w => w.questionnaireID == 12564).First();
                        ...
                        if (t_Script != null)
                        {
                            ...
                            answerList = (from element in variableList.Elements()
                                          select new AnswerList
                                          {
                                              ...
                                              Title = element.Value,
                                              Answer = GetAnswerData(AnswerData, element.Name.ToString()),
                                              Url = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_3&pid={0}&uid={1}&hid={3}&t=tapi&eqnum={2}&nv=true", pid, UID, element.Name.ToString(), hid)),
                                          }).Where(w => string.IsNullOrEmpty(w.Answer) == false).ToList();
                        }
                    }
                }
        ...
        public ActionResult IncomeSuvey(long hid)
        {
            ...
            try
            {
                ...
                using (TAPIDataContext TAPI = new TAPIDataContext("Data Source=10.0.11.175,15000;Initial Catalog=TAPI;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                {
                    ...
                    string query = string.Format(@"IF EXISTS (SELECT * FROM SURVEY.sys.objects WHERE object_id = OBJECT_ID(N'[SURVEY].[dbo].[T_SamplingList_q220355_6]') AND type in (N'U'))
                    BEGIN
                        MERGE SURVEY.dbo.T_SamplingList_q220355_6 as SLIST
                        USING
                        (
                            SELECT	'{0}' as PID
                        ) as Answer on SLIST.PID = Answer.PID
                ...
                if (eqnum == false)
                {
                    ViewBag.C = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_6&t=tapi&pid={0}&uid={1}&splith={2}&nv=true", hid, UID, splitH));
                }
                else
                {
                    string dataFile = string.Format(@"{0}{1}.hrcdata", Server.MapPath("../").Replace(@"TAPI", @"DATA\12565").Replace("/q220355_6", ""), hid);

                    //dataFile = @"D:\MyProject\서베이툴\RPS_SurveyEngine\DATA\11582\" + pid + ".xml";
                    ...
                    using (SURVEYTOOLDataContext SURVEYTOOL = new SURVEYTOOLDataContext("Data Source=10.0.11.175,15000;Initial Catalog=SURVEYTOOL;Persist Security Info=True;User ID=kimsj;password=dv5000157$"))
                    {
                        ...
                        T_Script t_Script = SURVEYTOOL.T_Scripts.Where(w => w.questionnaireID == 12565).First();
                        ...
                        if (t_Script != null)
                        {
                            ...
                            answerList = (from element in variableList.Elements()
                                          select new AnswerList
                                          {
                                              ...
                                              Title = element.Value,
                                              Answer = GetAnswerData(AnswerData, element.Name.ToString()),
                                              Url = TripleDESCryptoService.Encrypt(string.Format("qn=q220355_6&pid={0}&uid={1}&t=tapi&eqnum={2}&splith={3}&nv=true", hid, UID, element.Name.ToString(), splitH)),
                                          }).Where(w => string.IsNullOrEmpty(w.Answer) == false).ToList();
                        }
                    }
                }
